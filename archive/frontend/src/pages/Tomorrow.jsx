...
function mostRecentTradingDayIso() {
  const d = new Date();
...
function isWeekendIso(iso) {
  // Parse a YYYY-MM-DD as a local date so we don't get a UTC off-by-one
  // when the operator's locale shifts the day.
  if (!iso) return false;
...
export default function Tomorrow() {
  const [dateStr, setDateStr] = useState(mostRecentTradingDayIso());
  ...
  const load = async (d) => {
    ...
    try {
      ...
      // The API may or may not return last_successful_date — defensive read.
      setLastSuccessful(body.last_successful_date || null);
    } catch (e) {
      setErr(e.message);
    } finally {
      setLoading(false);
  ...
  // If the operator (or default state) landed on a weekend, auto-redirect to
  // the most recent trading day so they don't stare at an empty page.
  useEffect(() => {
    if (!isWeekendIso(dateStr)) return;
    const recent = mostRecentTradingDayIso();
    if (recent !== dateStr) {
      const t = setTimeout(() => setDateStr(recent), 1200);
          ...
          {weekend ? (
            <>
              <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 6 }}>
                {dateStr} is a weekend — markets closed.
              </div>
              <div style={{ color: 'var(--muted)', fontSize: 13, marginBottom: 12 }}>
                Showing the most recent EOD pass below.
              </div>
            </>
          ) : (
            <>
              ...
              <div style={{ color: 'var(--muted)', fontSize: 13, marginBottom: 12 }}>
                Last successful EOD pass: {lastSuccessful || 'unknown'}.
                <br />Next EOD pass scheduled at 16:30 ET on weekdays.
