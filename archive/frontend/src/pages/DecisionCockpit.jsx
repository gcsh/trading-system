...
// Feature-Merge F3 — surface `would_have_been` projection on non-
// submitted decisions + a 3-card interactive Counterfactual What-If
// panel that hits the live /learning/counterfactual POST endpoints.
import WouldHaveBeenPanel from '../components/WouldHaveBeenPanel.jsx';
import CounterfactualWhatIfPanel from '../components/CounterfactualWhatIfPanel.jsx';
// Feature-Merge F3 — SWR-cached cockpit fetcher. Replaces the legacy
// inline useEffect+fetch so navigation between cockpits is instant
// (keepPreviousData) and the 8-18s cold cockpit endpoint doesn't block
// the UI on every nav. The legacy fetchJson is left in place because a
// downstream tool may import it; the page itself no longer uses it.
import { useDecisionCockpit } from '../hooks/swr/useDecisionCockpit.js';
...
export default function DecisionCockpit() {
  ...
  // Feature-Merge F3 — SWR-backed fetch.
  //   • keepPreviousData → navigation paints instantly with the
  //     previously-loaded cockpit until the new one resolves
  //     (the operator never stares at a blank screen for 8-18s).
  //   • dedup 30s → re-visiting the same decision within 30s is free.
  //   • isValidating → background refresh indicator (faint, non-blocking).
  // The cold cockpit endpoint is 8-18s on EC2 because the backend runs
  // compute_all_counterfactuals + 4 learning aggregations synchronously
  // per request (see backend/api/routes/decision.py:518, 530). Caching
  // here turns most navigations into instant renders; the only call
  // that pays the full backend cost is the very first cockpit fetch.
  const { cockpit: data, error: err, isLoading, isValidating } = useDecisionCockpit(identifier);

  // Sync the picker input when the URL identifier changes (e.g. user
  // navigates from the Hypothesis Studio link back into a cockpit).
  React.useEffect(() => { setPicked(identifier || ''); }, [identifier]);

  // Feature-Merge F3 — counterfactual recompute is decoupled from the
  // cockpit fetch so the operator can iterate sizing factors / rule
  // overrides without re-paying the 8s cockpit cost. Seeded with the
  // bundle already returned by /decision/cockpit so the 3 cards render
  // populated on first paint, then update as POSTs land.
  const provIdForCf = data?.decision_id || null;
  const {
    cf: cfBundle,
    computing: cfComputing,
    error: cfError,
    recompute: cfRecompute,
  } = useCounterfactual(provIdForCf, { initial: data?.counterfactuals || null });

  // Pull the live blocker rule list + known agent roster out of the
  // cockpit response so the Policy + Consensus dropdowns are real.
  const policyBlockers = React.useMemo(() => (
    (data?.policy_result?.blocking_factors || [])
      .map((f) => f?.rule)
      .filter(Boolean)
  ), [data]);
  const knownAgents = React.useMemo(() => (
    data?.learning_insights?.active_weight_proposals?.known_agents || []
  ), [data]);

  // F3 — skeleton header: always render the ticker pulled from the
  // URL (or the picker input) so the page never appears blank, even
  // on a true cold load with no cached prior cockpit.
  const skeletonTicker = (
    identifier && /^[A-Z\-.]{1,8}$/i.test(identifier)
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      {/* F3 — skeleton header. Renders when the page is loading the
          FIRST cockpit (no prior data cached). Once `data` exists,
          revalidation runs silently in the background (see the
          revalidating chip on the real header above) so we don't show
          a spinner on every nav. */}
      {identifier && isLoading && !data && (
        <div style={{
          background: '#111827', borderRadius: 8,
          padding: 24, border: '1px solid #1f2937',
          marginBottom: 16,
          ...
          <div style={{
            ...
            Loading decision cockpit{skeletonTicker ? ` for ${skeletonTicker}` : ''}…
          </div>
          <div style={{ color: '#9ca3af', fontSize: 12, lineHeight: 1.55 }}>
            The cockpit endpoint runs counterfactual replay + learning
            insights synchronously, so the first fetch is heavier than a
            normal page (8-18 seconds when the engine is busy).
            Subsequent cockpits are cached and load instantly.
          </div>
          <div style={{
            marginTop: 12, height: 4, background: '#1f2937',
            borderRadius: 2, overflow: 'hidden', position: 'relative',
          }}>
            <div style={{
              position: 'absolute', height: '100%', width: '30%',
              background: '#3b82f6',
              animation: 'f3CockpitPulse 1.6s ease-in-out infinite',
            }} />
          </div>
          <style>{`
            @keyframes f3CockpitPulse {
              0% { left: -30%; }
              100% { left: 100%; }
            }
          `}</style>
      ...
      {/* F3 — background-revalidation hint. Shows when a stale cockpit
          is on screen and SWR is fetching a fresh copy. Non-blocking. */}
      {data && isValidating && (
        <div style={{
      ...
      {data && (
        <>
          ...
          />
          {/* F3 — "would have been" projection for non-submitted
              decisions (HOLD / ABSTAIN / blocked). Renders the 4 plain-
              English text projections persisted on the provenance row.
              On submitted rows, shows an EmptyState pointing at the
              real execution panels below. */}
          <WouldHaveBeenPanel
            eventStatus={data.event_status}
            wouldHaveBeen={data.would_have_been}
          />
          ...
          {/* F3 — interactive 3-card what-if. Sits next to the static
              snapshot panel so the operator sees both "what's been
              precomputed" and "what can I run live". */}
